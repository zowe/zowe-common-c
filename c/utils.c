...
static void rc4Init(RC4State *rc4State){
  int i,j,temp;
  int *s = rc4State->s;
  char *key = rc4State->key;
  int len = rc4State->keyLen;
...
RC4State *createRC4(char *key, int len){
  RC4State *rc4State = (RC4State*)safeMalloc31(sizeof(RC4State),"RC4 State");
  int *s;
  int x;
  
  rc4State->key = key;
  rc4State->keyLen = len;
  s = (int*)safeMalloc31(256*sizeof(int),"RC4 StateVector");
  rc4State->s = s;
  rc4Init(rc4State);
  return rc4State;
}
...
void rc4Free(RC4State *rc4){
  safeFree31((char*)rc4->s,256*sizeof(int));
  safeFree31((char*)rc4,sizeof(RC4State));
}
...
char rc4Next(RC4State *rc4){
  ...
  s[rc4->i] = s[rc4->j];
  s[rc4->j] = temp;
  k = s[(s[rc4->i] + s[rc4->j]) % 256];
  if (k < 0 || k > 255){
    printf("key bytes should be UNSIGNED 8, not %d\n",k);
  }
  return (char)(k&0xff);
}
  
char rc4EncChar(RC4State *rc4, char b){
  return (char) (rc4Next(rc4) ^ b);
}
...
void rc4Encrypt(RC4State *rc4, char *src, int len, char *dst){
  ...
  for (i = 0; i < len; i++) {
    dst[i] = (char) (rc4Next(rc4) ^ src[i]);
  }
}

void rc4Decrypt(RC4State *rc4, char *src, int len, char *dst){
  rc4Encrypt(rc4,src,len,dst);  
}
