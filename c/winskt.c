...
Socket *tcpClient2(SocketAddress *socketAddress, 
  ...
  if (windowsSocket == INVALID_SOCKET){
    ...
    if (!madeConnection){
      if (socketTrace){
        printf("Failed to connect socket, will clean up, SOCKET at 0x%x\n",windowsSocket);
      }
      *returnCode  = 0;
      *reasonCode  = 0; 
      status = closesocket(windowsSocket);
      ...
      sprintf(socket->debugName,"HNDL=0X%x",socket->windowsSocket);
...
int socketWrite(Socket *socket, const char *buffer, int desiredBytes, 
  ...
  if (bytesWrittenOrError < 0){
    *returnCode = WSAGetLastError();
    *reasonCode = *returnCode;
    printf("write failed, socketHandle=0x%x,desired write len %d buffer at 0x%x, ret code %d reason 0x%x\n",
...
Socket *socketAccept(Socket *serverSocket, int *returnCode, int *reasonCode){
  ...
  printf("newSocketAddressSize=%d sizeof(SocketAddress)=%d\n",newSocketAddressSize,sizeof(SocketAddress));
  fflush(stdout);
...
int socketSetAdd(SocketSet *set, Socket *socket){
  ...
  if (socketTrace)
  {
    printf ("Adding socket, SOCKET HANDLE=0x%x\n", windowsSocket);
  }
...
int socketSetRemove(SocketSet *set, Socket *socket){
  ...
  if (socketTrace)
  {
    printf ("Removing socket, SOCKET HANDLE=0x%x\n", windowsSocket);
  }
