...
static xmlPrinter *makeXmlPrinterInternal(int isCustom, 
  ...
  zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "makeXMLPrinter custom=%d full 0x%x byte 0x%x\n",
...
xmlPrinter *makeHttpXmlPrinter(unsigned char *handle, char *xmlDeclaration){
  ...
  if (p == NULL){
    zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_SEVERE, "malloc fail in makeHttpXMLPrinter...\n");
    fflush(stdout);
  }
...
xmlPrinter *xmlStart(xmlPrinter *p, char *elementName){
  ...
  if (p->stackSize + 1 >= XML_STACK_LIMIT){
    zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_WARNING, "XML Printer stack limit exceeded\n");
    fflush(stdout);
  }
  if (p->stackSize > 15 || p->stackSize < 0){
    zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_WARNING, "deep /negative XML stack size %d seen while starting level for %s\n",p->stackSize,elementName);
    fflush(stdout);
  }
...
static void writeBAOS(ByteArrayOutputStream *baos, int b){
  if (baos->pos == (BAOS_MAX + baos->extraSize)){
    zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_SEVERE, "PANIC, baos out of room\n");
  }
...
static void syntaxError(XmlParser *p, char *formatString, ...){
  ...
  zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_SEVERE, "SYNTAX ERROR line=%d %s\n",(p ? p->lineNumber : -1) ,buffer);
}
...
XmlParser *makeXMLParserUSS(UnixFile *file){
  ...
  parser->hasPushback = FALSE;
  parser->stringInput = NULL;
  zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "about to return UnixFile based parser 0x%x with SLH at 0x%x and lineBuffer at 0x%x\n",parser,parser->slh,parser->lineBuffer);
  return parser;    
}
...
XmlParser *makeXmlParser(char *dcb){
  ...
  parser->hasPushback = FALSE;
  parser->stringInput = NULL;
  zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "about to return DCB based parser 0x%x with SLH at 0x%x and lineBuffer at 0x%x\n",parser,parser->slh,parser->lineBuffer);
  return parser;  
}
...
void printXMLToken(XMLToken *t){
  zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "<XMLToken %s text=\"%s\">",
...
static int safeRead(XmlParser *p) {
  if (p->hasPushback) {
    ...
    if (c == 0x15){
      p->lineNumber++;
      if (traceXML) {
        zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "read line number %d\n",p->lineNumber);
      }
    } else{
      if (traceXML) {
        zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "read '%c' on line %d\n",c,p->lineNumber);
      }
    }
    return c;
  }
...
static int sanityCheck(XmlParser *p, char *site){
  if ( (p->lineLength > 1800) && (p->lineBuffer[0x720] != 0x40)){
    zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_SEVERE, "sanity check failed at %s\n",site);
    return 0;
  } else{
    return 1;
  }
...
static int safeRead1(XmlParser *p) {
  ...
  if (p->hasPushback) {
    ...
    case READER_STATE_INITIAL:
      ...
      zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "getline1(), length=%d\n", p->lineLength);
	...
	p->eof = getline(p->dcb,p->lineBuffer,&(p->lineLength));
        zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "getline2() length=%d\n",p->lineLength);
        dumpbuffer(p->lineBuffer,p->lineLength);
        p->lineNumber++;
	p->readState = READER_STATE_LINE_MIDDLE;
	p->linePos = 1;
	return (p->eof ? -1 : p->lineBuffer[0]);
      ...
      break;
    default:
      zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "unknown state\n");
      break;
    }
  }
}
...
static int safeRead(XmlParser *p) {
  int c = safeRead1(p);
  if (c == 0x15){
    zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "newline to %d\n",p->lineNumber);
  } else{
    zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "'%c' seen on line number %d\n",c,p->lineNumber);
  }
  return c;
}
...
XMLToken *getXMLToken(XmlParser *p) {
  ...
  if (b == -1) {
	...
	if (isWhitespace(c)) {
      ...
      zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_WARNING, "help unknown token\n");
      return NULL;
    }
  } else { /* NOT TAG MODE*/
    writeBAOS(p->tokenBytes,b);
...
XMLToken *getTokenNoWS(XmlParser *p) {
  ...
  while (1) {
    t = getXMLToken(p);
    if (traceXML){
      zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "NO WS loopB type=%s, %s\n",
...
void pprintNode2(XMLNode *node, int indent) {
  ...
  switch (node->type) {
  case NODE_TEXT:
    for (i=0; i<indent; i++) zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, " ");
    zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "#text: '%s'\n",node->value);
    break;
  case NODE_ELEMENT:
    for (i=0; i<indent; i++) zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, " ");
    zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "<%s cc=%d",node->name,node->childCount);
    ...
    break;
  case NODE_ATTRIBUTE:
    zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, " %s=\"%s\"",node->name,node->value);
  }
}
...
void showStack(XmlParser *p, XMLNode** stack, int stackPointer) {
  ...
  for (i=stackPointer-1; i>=0; i--) {
    zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "%s ",stack[i]->name);
  }
  zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "\n");
}
...
XMLNode *parseXMLNode(XmlParser *p) {
  ...
  while (1) {
    ...
    if (traceXML){
      zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "Parse Loop top state %d '%s'tokenType=%d '%s'\n",
      ...
      case STATE_ATTRIBUTES:
        {
          ...
          XMLToken *nextToken = getTokenNoWS(p);
	  if (traceXML){
        zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "ATTR< equalsToken=%s valueToken=%s nextToken=%s\n",
...
#ifdef METTLE
      zowelog(NULL, LOG_COMP_UTILS, ZOWE_LOG_INFO, "junk linePos = 0x%x len=0x%x\n",p->linePos,p->lineLength);
