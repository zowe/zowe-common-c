...
int stcBaseSelect(STCBase *stcBase,
  ...
  if ((stcBase->currentEventSet == NULL) ||
    ...
    for (int i=0; i<currentEventSetSize; i++){
      ...
      if (i==0){
        ...
        printf("eventSelectStatus=%d fpor socket handle=0x%x\n",eventSelectStatus,socket->windowsSocket);
        if (eventSelectStatus != 0){
          printf("WSAEventSelectFailed status=%d WSA lasteError=%d\n",eventSelectStatus,WSAGetLastError());
          fflush(stdout);
  ...
  if (failedToBuildEventSet){
    printf("*** WARNING FAILED TO BUILD EVENT SET\n");
    return STC_BASE_SELECT_FAIL;
  } 
  ...
  printf("waitForMultiple status=%d\n",status);
  if (status == WAIT_TIMEOUT){
    printf("woke up due to timeout\n");
    fflush(stdout);
    return STC_BASE_SELECT_TIMEOUT;
  } else if (status == -1){
    printf("WaitForMultipleObjects status=%d WSA lasteError=%d\n",status,GetLastError());
    fflush(stdout);
    return STC_BASE_SELECT_FAIL; 
    ...
    printf("FD_READ=0x%x FD_WRITE=0x%x FD_ACCEPT=0x%x\n",FD_READ,FD_WRITE,FD_ACCEPT);
    for (int j=1; j<currentEventSetSize; j++){
      ...
      printf("enum for j=%d status = %d for socket handle 0x%x -> networkEvents=0x%x\n",
    ...
    printf("thread woke up becase event %d was abandoned, which isn't really a failure, but needs a new status to clean up sockets\n",status);
    return STC_BASE_SELECT_FAIL; 
  } else {
    printf("What is this status=%d\n",status);
    return STC_BASE_SELECT_FAIL;
  }
}
...
static void extract(IEZCOM * __ptr32 * __ptr32 iezcom)
{
...
#ifdef DEBUG
  printf("EXTRACT(2) parameters at 0x%p\n",parameters);
  ...
  __asm(
        ...
        : "r15");
#ifdef DEBUG
  printf("EXTRACT parms after SVC\n");
  dumpbuffer((char*)parameters,sizeof(ExtractParameters));
#endif
}
