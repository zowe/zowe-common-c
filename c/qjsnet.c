...
static char certVerifyASCII[11] ={0x63, 0x65, 0x72, 0x74, 0x56, 0x65, 0x72, 0x69, 0x66, 0x79,  0x00};
...
static JSValue netTLSClientConfig(JSContext *ctx, JSValueConst this_val,
  ...
  if (!JS_IsUndefined(certVerify)){
    ...
    if (certVerifyText && 0 == strcasecmp(certVerifyText, "strict")) {
      defaultTLSSettings->certVerify = TLS_CERTVERIFY_STRICT;
    } else if (certVerifyText && 0 == strcasecmp(certVerifyText, "nonstrict")) {
      defaultTLSSettings->certVerify = TLS_CERTVERIFY_NONSTRICT;
    } else if (certVerifyText && 0 == strcasecmp(certVerifyText, "disabled")) {
...
static int httpGet(bool isTLS,
  ...
  do{
    ...
    status = httpClientSessionInitv2(httpClientContext, &session, &rc);
    if (status){
      if (httpTrace){
