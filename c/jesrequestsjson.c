...
void responseWithSubmitJCLContents(HttpResponse* response, char* jclLines) {
  ...
  char *jobID;
  char lastJobID[JOB_ID_LEN + 1] = JOB_ID_EMPTY;
  char firstJobID[JOB_ID_LEN + 1] = JOB_ID_EMPTY;
  char *tJCL;
  int recordLength;
  tJCL = jclLines;
  for ( i = 0; i < j; i++) {
  ...
  for (int i = 0; i < j; i++) {
    ...
    if (memcmp(firstJobID, JOB_ID_EMPTY, JOB_ID_LEN) == 0) {
        memcpy(firstJobID, jobID, JOB_ID_LEN);
        zowelog(NULL, LOG_COMP_RESTDATASET, ZOWE_LOG_DEBUG,  "firstJobID (%-8.8s)\n", firstJobID);
        }
  ...
  zowelog(NULL, LOG_COMP_RESTDATASET, ZOWE_LOG_DEBUG,  "firstJobID_Length (%d)\n", strlen(firstJobID));
  if (!strlen(firstJobID)) {
	...
	responseWithJobDetails(response, firstJobID, TRUE);
  }
...
void responseWithSubmitJobs(HttpResponse* response, int jsonMode) {
  ...
  CVT *theCVT = getCVT();
  struct jesct *theJESCT = (struct jesct*)(theCVT->cvtjesct);
  zowelog(NULL, LOG_COMP_RESTDATASET, ZOWE_LOG_DEBUG, "JESCT eyecatcher...(%-4.4s)\n", theJESCT->jesctid);
  zowelog(NULL, LOG_COMP_RESTDATASET, ZOWE_LOG_DEBUG, "JES Version...(%x)\n", theJESCT->jesjesfg);

  if (CHECK_BIT(theJESCT->jesjesfg, 5)) {
    zowelog(NULL, LOG_COMP_RESTDATASET, ZOWE_LOG_DEBUG, "JES 3 system\n");
    respondWithMessage(response, HTTP_STATUS_INTERNAL_SERVER_ERROR,
      "BAD_REQUEST failed with RC = %d, Submit to JES3 subsystem is not supported.", 8);
  }
  else {
    zowelog(NULL, LOG_COMP_RESTDATASET, ZOWE_LOG_DEBUG, "JES 2 system\n");
  }
