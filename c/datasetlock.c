...
static char* makeStringCopy(char *from) {
  ...
  char* to=(char*)safeMalloc((len)*sizeof(char));
  strcpy(to, from);
  return to;
}
...
static void freeKeySemTable(void* key) {
  safeFree((DsnMember*)key, sizeof(key));
}

static void freeValueSemTable(void *value) {
  SemEntry *entry = value;
  safeFree((SemEntry*)value, sizeof(entry));
}
...
static int searchUserInSem(char* user) {
  ...
  for (int j = 0; j < backboneSize; j++) {
    ...
    if (strcmp(value->user, user) == 0) {
      if (value->semId > 0) {
        zowelog(NULL, LOG_COMP_RESTDATASET, ZOWE_LOG_DEBUG2,"heartbeat expire user: %s dataset: %s %s semId: %d \n", value->user, value->dsnMember.dsn, value->dsnMember.membername, value->semId);
...
static int fillSemTableEntry(int semaphoreID, DsnMember *dsnMember, char* username, SemEntry** entryPtr) {
  SemEntry* entry;
...
int findSemTableEntryByDatasetByUser(DsnMember *dsnMember, char* username, SemEntry** entryPtr) {
  SemEntry* entry;
  int ret=findSemTableEntryByDataset(dsnMember, &entry);
  if (ret == 0 && entry!=NULL) {
...
static int acquireSemTableSlot(DsnMember *dsnMember, char* username, SemEntry** entryPtr) {
  SemEntry *entry;
  int ret=findSemTableEntryByDatasetByUser(dsnMember, username, &entry);
  
  if (ret == NO_MATCH_DSN) {
    ret = fillSemTableEntry(0, dsnMember, username, &entry);
  } 
...
static int releaseSemTableSlot(SemEntry* entry) {
...
int semTableEnqueue(DsnMember *dsnMember, char *username, SemEntry** entryPtr) {
  ...
  pthread_mutex_lock(&lockService->datasetMutex);
  {
    ret=acquireSemTableSlot(dsnMember, username, &entry);
  }
  pthread_mutex_unlock(&lockService->datasetMutex);
