/*
  This file has been superseded. Tests for json.h now live in:

    tests/unit/json.c

  Please see tests/README.md for the directory structure and
  contribution guidelines for this test suite.
*/
...
static void testJsonWriterBufferPrinter(void) {
  DESCRIBE("JSON Writer - buffer printer") {
    ...
    IT("jsonBufferRewind allows the buffer to be reused") {
      ...
      /* Buffer length should be similar to the first write */
      ASSERT_GT_INT(buf->len, 0);
