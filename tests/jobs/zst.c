...
/* ----------------------------------------------------------------
   Build flat array from linked list
   ---------------------------------------------------------------- */
...
static void buildJobArray(STData *data) {
  ...
  if (data->jobCount <= 0) return;

  data->jobArray = (JobInfo **)malloc(data->jobCount * sizeof(JobInfo *));
  if (!data->jobArray) return;

  JobInfo *j = data->jobList;
  for (int i = 0; i < data->jobCount && j; i++, j = j->next) {
    data->jobArray[i] = j;
  }
}
