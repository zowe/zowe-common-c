...
int main(int argc, char *argv[]) {
  ...
  if (filter[0]) {
    snprintf(tv.title, sizeof(tv.title), "SYSLOG [filter: %s] (%d lines)",
             filter, coll.lineCount);
  } else {
    snprintf(tv.title, sizeof(tv.title), "SYSLOG (%d lines)", coll.lineCount);
  }
  tv.lines = coll.lines;
  tv.lineCount = coll.lineCount;

  /* Start at end if requested */
  if (startAtEnd) {
    /* tuiTextViewShow will set scrollRow based on screen size,
       but we pre-set to near the end */
    tv.scrollRow = coll.lineCount;  /* will be clamped in viewer */
  }

  /* Need to init terminal for text viewer */
  TuiTable dummyTui;
