...
## `3.6.0`
- Enhancement: `getEDB`, `getOCB` and `getLEHeapOptions` in `le.h` read Language Environment's options control block, so code can ask whether `HEAPPOOLS`, `HEAPPOOLS64` or `HEAPZONES` is in effect instead of inferring it from heap behaviour. Comes with `tests/le-options`. [(#678)](https://github.com/zowe/zowe-common-c/pull/678)
