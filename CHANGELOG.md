...
## `3.6.0`
- Bugfix: `fileCopyConverted()` opened the destination with `FILE_OPTION_CREATE_IF_NON_EXISTENT`, which is `O_CREAT | O_EXCL`, so copying onto an existing file failed with `EEXIST` (surfaced by ZSS as `rc=14`, HTTP 500) even when overwrite was intended. It now uses `FILE_OPTION_CREATE` (`O_CREAT`), letting `O_TRUNC` overwrite an existing destination.
