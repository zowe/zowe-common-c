...
## `3.6.0`
- Bugfix: `${{ ... }}` templates in a configuration file are now evaluated against a restricted JavaScript surface. Only the read-only members of `std`, `os` and `zos` reach `globalThis` (`getenv`, `getenviron`; `realpath`, `stat`, `lstat`, `readlink`, `readdir`, `getcwd`, `platform`; `zstat`, `getZosVersion`, `getEsm`, `dslist`, `resolveSymbol`, `getStatvfs` and the two `EXTATTR_` constants), and the `__loadScript` helper that `js_std_add_helpers()` leaves on the global object is removed. Previously the whole of each module was published unconditionally, so a template could call `os.exec`, `std.popen`, `std.loadScript` or `zos.changeExtAttr`: anyone able to write a file or PARMLIB member on the config path had arbitrary command execution as the reading program's user, with no SAF or file-ownership check, and before the configuration was checked against its schema -- template evaluation happens inside `cfgLoadConfiguration()`, and `cfgLoadSchemas()`/`cfgValidate()` only run after it. Anything that starts a process, evaluates code, writes to the filesystem or its security attributes, or changes process state is withheld, and a member added to one of these modules later is withheld until it is named. Scripts run with `configmgr -script`/`-embeddedScript` are unaffected and still get the full modules; the new `configureEmbeddedJSForTemplates()` in `embeddedjs.c` applies only to the ConfigManager's own template runtime.
...
- Bugfix: the logging component-ID walk reads the ID's four shorts by host byte order: in place on big-endian hosts, exactly as before, and extracted with shifts elsewhere. Aliasing the 64-bit value read the levels in reverse on little-endian hosts and, in `logShouldTraceInternal()`, made the vendor check read the low short, so on Linux a level-1 component's configuration landed on the root and every zowe component started tracing. Comes with `tests/logging`. [(#668)](https://github.com/zowe/zowe-common-c/issues/668)
...
- Bugfix: Fixed a potential memory bug in CSI function `returnEntries`. [(#711)](https://github.com/zowe/zowe-common-c/pull/711)
