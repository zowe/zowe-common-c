...
## `3.6.0`
- Bugfix: `${{ ... }}` template expressions in a configuration file can no longer reach the QuickJS `os` and `std` globals. `configureEmbeddedJS()` publishes `std`, `os` and `zos` on `globalThis` so that non-module code can use them, and templates are evaluated as non-module code, so any value in `zowe.yaml` could call `os.exec()` or `std.popen()` and run a command in the address space and under the identity of whichever program loaded the configuration -- for the launcher, an APF-authorized started task, before the configuration had been validated against a schema. `evaluateJsonTemplates()` now withdraws the two globals for the duration of template evaluation and restores them after. `zos` remains available because `zos.getEsm()` is evaluated by the shipped `defaults.yaml`; scripts run through `-script` are unaffected because they import `cm_std` / `cm_os` rather than using the globals.
