...
## `3.6.0`
- Bugfix: the POSIX `fileReadLink()` in `platform/posix/psxfile.c` takes `const char *` like its declaration in `unixfile.h`, so the Linux build of configmgr compiles again after #643.
