...
## `3.6.0`
- Bugfix: `directoryMakeDirectoryRecursive()` now rejects a path that does not fit its internal buffer instead of overrunning a fixed 256-byte stack buffer, which caused a 0C4 ABEND (crash) when creating a directory with a path longer than 255 characters. [(zss#2094)](https://github.com/zowe/zss/issues/2094)
