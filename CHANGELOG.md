...
- Bugfix: the inline `TROO` translate in `charsets.c` (Open XL builds) now declares the registers it loads, R0, R1, R2, R8, R9 and R15, in its clobber list; without them the compiler could keep live values in those registers across the call. [(#644)](https://github.com/zowe/zowe-common-c/issues/644)
...
- Bugfix: `console.log()` in embedded JS no longer truncates at 64KB. `js_native_print()` writes longer text in successive 64KB chunks instead of dropping the remainder, short text (nearly all of it) uses a 256-byte stack buffer rather than allocating 64KB on every call, and the heap buffer is allocated only when it is needed and checked before use. [(#716)](https://github.com/zowe/zowe-common-c/issues/716)
