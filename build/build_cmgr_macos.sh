...
# Build script for configmgr on macOS using clang.
# This is a macOS-only companion to build_cmgr_xlclang.sh (which targets z/OS).
...
# -DYAML_DECLARE_STATIC : link libyaml statically (no .dylib needed)
# -------------------------------------------------------------------------------
...
# Extra flags applied only when compiling QuickJS sources.  quickjs.c contains
# a bare 'typedef int ssize_t' (for 32-bit non-z/OS targets) that conflicts
# with Darwin's ssize_t = long on 64-bit.  We pre-include a shim that pulls
# in <sys/types.h> first (defining ssize_t as 'long'), then use
# -Wno-typedef-redefinition to turn the subsequent redundant typedef into a
# warning rather than an error, and -Wno-error to prevent that warning from
# aborting the compilation.
QJS_EXTRA_CFLAGS="-Wno-typedef-redefinition -Wno-error=typedef-redefinition"
QJS_EXTRA_CFLAGS="${QJS_EXTRA_CFLAGS} -include ${COMMON}/platform/posix/quickjs_macos_compat.h"
...
# QuickJS sources (portable fork; no z/OS polyfill needed on macOS)
QJS_SRCS="
  ${QJS}/cutils.c
  ${QJS}/quickjs.c
  ${QJS}/quickjs-libc.c
...
# to omit entirely.
#
# qjszos.c and qjsnet.c use z/OS-specific headers that do not exist on macOS;
# we replaced them with stub_zos_modules.c.
#
...
# therefore omit those modules; if TLS or HTTP support is added later, they
# can be re-enabled here along with the appropriate -lssl / -lz flags.
COMMON_SRCS="
  ${COMMON}/c/alloc.c
  ${COMMON}/c/charsets.c
...
# Compile QuickJS sources separately with the extra flags that suppress
# the bare 'typedef int ssize_t' in quickjs.c which clashes with Darwin's
# ssize_t (which is 'long' on 64-bit ARM/x86).
QJS_OBJS=""
for src in ${QJS_SRCS}; do
  base=$(basename "$src" .c)
  obj="${TMP_DIR}/${base}.o"
  # shellcheck disable=SC2086
  clang ${CFLAGS} ${QJS_EXTRA_CFLAGS} -c -o "${obj}" "${src}"
  rc=$?
  if [ "${rc}" -ne 0 ]; then
...
for src in ${COMMON_SRCS}; do
  base=$(basename "$src" .c)
  # Guard against name collisions across directories
...
echo "Linking..."
# shellcheck disable=SC2086
clang ${CFLAGS} -o "${OUTPUT}" ${QJS_OBJS} ${YAML_OBJS} ${COMMON_OBJS} -lm -liconv
rc=$?
